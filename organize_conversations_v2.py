...
def extract_and_format_conversation(conversation_data):
        ...
        for node_id, node in conversation_data["mapping"].items():
            if node and "message" in node and node["message"]:
                ...
                for part in content_parts:
                    ...
                    # Ignorer les autres types de contenu comme les images pour le moment

                if text_content.strip(): # N'ajouter que les messages non vides
                    messages.append({
                        "role": author_role,
                        "time": create_time,
    ...
    messages.sort(key=lambda x: x["time"] if x["time"] is not None else 0)
...
def organize_conversations_v2(json_file_path, output_base_dir):
    """
...
if __name__ == "__main__":
    ...
    output_base_directory = "/home/luciedefraiteur/ShadEOS/bible-des-en-faire/disclosure-conversations-chronology-v2/"
    organize_conversations_v2(json_input_path, output_base_directory)
